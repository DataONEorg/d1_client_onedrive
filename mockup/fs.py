...
def get_random_words_sorted(n_words):
  words = [re.sub(r'[^a-z]', '', line.strip().lower()) for line in open('/etc/dictionaries-common/words')]
...
keyword_group = [(f, group_from_first_last(random_words, f), d) for f in first_last(random_words, 100)]
...
member_node_restriction_mockup = tuple((m, unapplied_facets + objects_unfiltered, d) for m in member_nodes)
...
countries_tuple = tuple([(c, unapplied_facets + objects_unfiltered, d) for c in countries])
...
countries_all_tuple = tuple([(c, unapplied_facets + objects_unfiltered, d) for c in countries_all])
