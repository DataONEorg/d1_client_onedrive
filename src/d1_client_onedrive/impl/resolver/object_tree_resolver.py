...
class Resolver(resolver_base.Resolver):
  ...
  def get_attributes(self, object_tree_root, path):
    ...
    # the path is invalid.
    try:
      object_tree_folder = self._object_tree.get_folder(object_tree_path, object_tree_root)
    except ONEDriveException:
  ...
  def read_file(self, object_tree_root, path, size, offset):
    ...
    return self._resolvers[root_name].read_file(object_tree_folder, controlled_path,
                                              size, offset)
