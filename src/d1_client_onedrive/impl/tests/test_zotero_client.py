...
class TestZoteroClient(unittest.TestCase):
  ...
  def _copy_dirs_recursive(self, zotero_client, object_tree_client, zotero_collection, object_tree_path):
    object_tree_folder = object_tree_client.get_folder(object_tree_path)
    print object_tree_folder['name']
    collection = zotero_create_collection(zotero_client, zotero_collection, object_tree_folder['name'])
    for pid in object_tree_folder['items']:
      zotero_create_item(zotero_client, collection, pid)
    for d in object_tree_folder['dirs']:
      copy_dirs_recursive(zotero_client, object_tree_client, collection, object_tree_path + [d])
  ...
  def setUp(self):
    ...
    self.pickle_path = os.path.join(self.options['zotero_cache_root'],
                                    self.options['zotero_cache_path'])
