...
class ZoteroClient(object):
  ...
  def cache_is_stale(self):
    ...
    logging.debug('Zotero online version: {}. Cached version: {}'.format(
  ...
  def _create_filtered_trees_from_collections_recursive(self, filtered_tree, collection_tree):
    sub_tree = {
      'collections': {},
    }
  ...
  def _get_filtered_sub_tree_recursive(self, path, filtered_tree=None):
    ...
    if not path:
      return filtered_tree
    try:
      return self._get_filtered_sub_tree_recursive(path[1:], filtered_tree['collections'][path[0]])
    except KeyError:
