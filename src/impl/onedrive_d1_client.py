...
class D1Client(object):
  ...
  def get_unapplied_facet_names(self, applied_facets):
    ...
    applied_facet_names = set([self.facet_path_parser.undecorate_facet_name(f[0])
  ...
  def init_query_engine_description(self):
    self.query_engine_description = self.client.getQueryEngineDescription(
      'solr')
...
# UNUSED
  ...
  def get_science_object(self, pid):
    ...
    self.objectcache[pid] = d1_client.d1client.DataONEObject(pid,
      cnBaseUrl=self.base_url)
    return self.objectcache[pid]
