...
class Resolver(resolver_abc.Resolver):
  def __init__(self, command_processor):
    ...
    self.classifications = ['kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'species', 'scientificName']
  ...
  def _get_directory(self, path, workspace_folder_objects):
    ...
    return self._resolve_taxa_classification_value(classification,
      classification_value, workspace_folder_objects)
  ...
  def _resolve_taxa_classification(self, classification, workspace_folder_objects):
    dir = directory.Directory()
    self.append_parent_and_self_references(dir)
    u = self._get_unique_values_for_classification(classification, workspace_folder_objects)
    print u
    return [directory_item.DirectoryItem(v) for v in u]


  def _resolve_taxa_classification_value(self, classification, value, workspace_folder_objects):
    dir = directory.Directory()
    for o in workspace_folder_objects.objects:
      try:
  ...
  def _get_unique_values_for_classification(self, classification, workspace_folder_objects):
    u = set()
    for o in workspace_folder_objects.objects:
      try:
