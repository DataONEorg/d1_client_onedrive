...
class RootResolver(resolver_abc.Resolver):
  ...
  def resolve_root(self):
    ...
    directory.extend([DirectoryItem(r, 0, True) for r in
  ...
  def escape_directory_entries(self, directory):
    return Directory(DirectoryItem(os_escape.filename_from_identifier(
      d.name()), d.size(), d.is_directory()) for d in directory)
  ...
  def resolver_lookup(self, p):
    try:
      ...
      return self.render_error_message_as_file_in_directory(
