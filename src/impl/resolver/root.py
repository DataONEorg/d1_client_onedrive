...
class RootResolver(resolver_abc.Resolver):
  ...
  def __init__(self, options):
    ...
    self.resolvers = {
      RootResolver.FLDR_WORKSPACE: 
        workspace.Resolver(options, self.command_processor),
      RootResolver.FLDR_FLATSPACE: 
        flat_space.Resolver(options, self.command_processor),
    }
...
#      return self._render_path_exception_as_file(e.message)
  ...
  def _escape_directory_entries(self, dir):
    return directory.Directory(directory_item.DirectoryItem(
      os_escape.filename_from_identifier(d.name())) for d in dir)
  ...
  def _render_path_exception_as_file(self, path_exception):
    dir = directory.Directory()
    self.append_parent_and_self_references(dir)
    dir.append(directory_item.DirectoryItem(
