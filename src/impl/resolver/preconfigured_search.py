...
class Resolver(resolver_abc.Resolver):
  ...
  def get_attributes(self, path):
    ...
    if self._is_root(path):
      return attributes.Attributes(is_dir=True,
                                   size=len(settings.PRECONFIGURED_SEARCHES))
  ...
  def get_directory(self, path):    
    ...
    return self.faceted_search.get_directory(path[1:],
                                             preconfigured_query=query)
  ...
  def _resolve_root(self):
    ...
    dir.extend([directory_item.DirectoryItem(name) for name in
               sorted(settings.PRECONFIGURED_SEARCHES)])
    return dir
