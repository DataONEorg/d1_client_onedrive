...
class Resolver(resolver.Resolver):
  def __init__(self, query_engine):
    ...
    self.facet_path = facet_path.FacetPath()


  def resolve(self, path):
    facet_section, object_section = self.facet_path \
    ...
    #facets = self.facet_path.undecorate_facets(path)
    #if self.facet_path.dir_contains_facet_names(path):
    #  return self.resolve_dir_containing_facet_names(path, facets)
    #if self.facet_path.dir_contains_facet_values(path):
  ...
  def append_facet_directories(directory, facet_section):
    facets = self.facet_path.undecorate_facets(facet_section)
  ...
  def append_dir_containing_facet_values(self, directory, path, facets):
    directory = Directory()
    facet_name = self.facet_path.undecorated_tail(path)
  ...
  def append_facet_name_selection_directories(self, directory, facets):
    ...
    for facet_count in facet_counts:
      facet_name = self.facet_path.decorate_facet_name(facet_count[0])
  ...
  def append_facet_value_selection_directories(self, directory, objects, facets, facet_name):
    ...
    for facet_value_count in facet_value_counts:
      facet_value = self.facet_path.decorate_facet_value(facet_value_count[0])
