...
class SolrClient(object):
  def __init__(self, base_url=d1_common.const.URL_DATAONE_ROOT, solr_path=None,
               filter_query=None):
    '''
    ...
    self.solr_connection = d1_client.solr_client.SolrConnection(host=base_url,
      solrBase=solr_path, debug=True)
  ...
  def create_solr_connection(self, forceNew=True):
    ...
    if forceNew or self.solr_connection is None:
      ...
      print '0'*100
      self.solr_connection = solr_client.SolrConnection(self.get_solr_host(),
        solrBase=self.solr_path, persistent=settings.SOLR_PERSIST_CONNECTION)
  ...
  def _get_unused_facet_names(self, path):
    ...
    searchable_facet_names = set(self.query_engine_description.get_searchable_facet_names())
    used_facets = self.facet_path_parser.undecorate_facets(path)
    used_facets_names = set([self.facet_path_parser.undecorate_facet_name(f[0])
  ...
  def get_records(self, facet, term):
    ...
    records = solr_client.SOLRArrayResponseIterator(sc, q, fq=self._filter_query,
                                                   cols=self.fields,
