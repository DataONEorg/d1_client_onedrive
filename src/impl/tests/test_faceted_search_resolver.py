...
class TestFacetedSearchResolver(unittest.TestCase):
  def setUp(self):
    self.i = faceted_search_resolver.Resolver(
      solr_query_simulator.SolrQuerySimulator())
  ...
  def test_120_resolve(self):
    ...
    for o in d:
      self.assertTrue('square' in o.name() or
  ...
  def test_130_resolve(self):
    ...
    for o in d:
      self.assertTrue(o.name().startswith('red-square') or
