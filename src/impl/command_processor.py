...
class CommandProcessor():
  def __init__(self, options):
    ...
    self._solr_query_cache = cache_disk.DiskCache(self._options.MAX_SOLR_QUERY_CACHE_SIZE, './cache_solr')
  ...
  def _get_query_from_cache(self, query, filter_queries, fields):
    return self._solr_query_cache[
      self._dict_key_from_solr_query(query, filter_queries, fields)]


  def _cache_query_response(self, response, query, filter_queries, fields):
    self._solr_query_cache[
      self._dict_key_from_solr_query(query, filter_queries, fields)] = response
